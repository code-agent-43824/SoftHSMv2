...
if ($args.Count -lt 1 -or $args.Count -gt 2) {
    throw "usage: run-test.ps1 <softhsm-portable-package.zip> [scenario-directory]"
}
...
Write-Host "[TEST-KIT] product package=$Archive"
Write-Host "[TEST-KIT] precompiled client=$Client"
Write-Host "[TEST-KIT] bundled OpenSSL=$OpenSSL"

$Arguments = @($Archive, $Settings.MODULE_NAME, $OpenSSL)
